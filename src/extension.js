...
function activate(context) {
    ...
    // * Save File //
    const saveFile = vscode.commands.registerCommand('savedToBackup.saveFile', async (uri) => {
        if (!backupDirectory) {
        ...
        let document;
        let workspace; // Declare workspace here
        if (uri) {
        ...
        const date = (0, moment_1.default)().format('MMMDD');
    ...
    // * fromExplorer
    const backupFilesFromExplorer = vscode.commands.registerCommand('savedToBackup.backupFilesFromExplorer', async (...args) => {
    ...
    async function backupFile(uri, workspace) {
        try {
            ...
            const ext = path.extname(originalName);
            const baseName = path.basename(originalName, ext);
            const timestamp = (0, moment_1.default)().format('MMMDD_hmma').toLowerCase();
    ...
    const restoreFile = vscode.commands.registerCommand('savedToBackup.restoreFile', async () => {
        ...
        const backupItems = registry[originalFileName].map(backup => ({
            label: backup.fileName,
            description: `Date: ${(0, moment_1.default)(backup.date).format('MMMM Doராஜ், h:mm:ss a')}`,
    ...
    const restoreSnippet = vscode.commands.registerCommand('savedToBackup.restoreSnippet', async () => {
        ...
        const backupItems = snippetBackups.map(backup => ({
            label: backup.fileName,
            description: `Date: ${(0, moment_1.default)(backup.date).format('MMMM Doராஜ், h:mm:ss a')}`,
    ...
    const projectBackup = vscode.commands.registerCommand('savedToBackup.projectBackup', async () => {
        ...
        const date = (0, moment_1.default)().format('YYYY-MM-DD_HH-mm-ss'); // e.g., 2024-12-25_12-30-00
        ...
        try {
            ...
            const archive = (0, archiver_1.default)('zip', { zlib: { level: 9 } }); // Maximum compression
            ...
            output.on('close', () => {
                vscode.window.showInformationMessage(`Project backed up successfully to ${backupFilePath} (${archive.pointer()} total bytes)`);
            });
    ...
    const deleteOldBackups = vscode.commands.registerCommand('savedToBackup.deleteOldBackups', async () => {
        ...
        const cutoffDate = (0, moment_1.default)().subtract(hours, 'hours');
        ...
        for (const originalFile in registry) {
            if (registry.hasOwnProperty(originalFile)) {
                const updatedEntries = registry[originalFile].filter((backup) => {
                    const backupDate = (0, moment_1.default)(backup.date);
        ...
        try {
            ...
            let allRegistries = context.globalState.get('backupRegistries') || {};
    ...
    const exportBackups = vscode.commands.registerCommand('savedToBackup.exportBackups', async () => {
        ...
        try {
            if (exportPath.endsWith(".zip")) {
                ...
                output.on('close', () => {
                    vscode.window.showInformationMessage(`Project backed up successfully to ${exportPath} (${archive.pointer()} total bytes)`);
                });
                archive.pipe(output);
...
function getRegistry(context, workspace) {
    const workspaceKey = getWorkspaceKey(workspace);
    const allRegistries = context.globalState.get('backupRegistries') || {};
    return allRegistries[workspaceKey] || {};
}
function updateRegistry(context, workspace, originalFile, backupFile, backupFilePath) {
    const workspaceKey = getWorkspaceKey(workspace);
    let allRegistries = context.globalState.get('backupRegistries') || {};
...
function watchBackupDirectory(context, workspace) {
    if (!backupDirectory)
        return;
...
function deleteBackupFile(context, workspace, backupFilePath) {
    const workspaceKey = getWorkspaceKey(workspace);
    let allRegistries = context.globalState.get('backupRegistries') || {};
    let registry = allRegistries[workspaceKey] || {};
    for (const originalFile in registry) {
